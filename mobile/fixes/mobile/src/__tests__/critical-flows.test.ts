...
describe('Flow 1: Create budget -> Add expense -> Budget spent updates', () => {
  it('updates budget spent amount when an expense is created', async () => {
    ...
    // Budget period should be fetched
    expect(getMemberBudgetPeriod).toHaveBeenCalledWith(ALEX_ID, HOUSEHOLD_ID);

    // Budget spent amount should be updated: 0 + 87.35 = 87.35
    expect(updateBudgetSpentAmount).toHaveBeenCalledWith(
      ALEX_ID,
      'cat-groceries',
      '2026-01-25', // period start
      87.35
    );
  });
...
describe('Flow 2: Shared expense -> Split creation -> Settlement balance', () => {
  it('creates splits when shared expense is added, then calculates debt', async () => {
    ...
    // getUnsettledSharedExpenses queries:
    mockQueryOnce
