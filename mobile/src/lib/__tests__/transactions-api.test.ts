...
describe('createTransaction', () => {
  ...
  describe('budget update on expense', () => {
    it('triggers budget spent amount update for expense transactions', async () => {
      ...
      // Should succeed and create transaction
      expect(result.success).toBe(true);

      // Should have updated budget spent amount (budget update is handled via prefetchBudgetForAtomicUpdate)
      // The db.transact call should include budget update operation
      expect(db.transact).toHaveBeenCalled();
    });
...
describe('deleteTransaction', () => {
  ...
  it('returns error when account not found', async () => {
    ...
    expect(result.success).toBe(false);
    // Generic error message from catch block (Account not found throws and gets caught)
