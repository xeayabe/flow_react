...
function RootLayoutNav({ colorScheme }: { colorScheme: 'light' | 'dark' | null | undefined }) {
  ...
  useEffect(() => {
    ...
    if (!user && (inAuthGroup || inAccountsFlow || inSettingsFlow || inTransactionsFlow || inAnalyticsFlow)) {
      // Redirect to welcome if not authenticated
      router.replace('/welcome');
    } else if (user && !inAuthGroup && !inAccountsFlow && !inSettingsFlow && !inTransactionsFlow && !inAnalyticsFlow && segments[0] !== 'modal') {
